...
!function() {
    ...
    $.widget("ui.splitter", $.ui.mouse, {
        ...
        _create: function() {
            ...
            //this.header='<div class="ui-pane-header"></div>';
            //this.footer='<div class="ui-pane-footer"></div>';
        ...
        _initPanes: function() {
            ...
            this.element.addClass("ui-splitter")
                ...
                .each(function(i, pane) {
                    that._initDiv(pane,panesConfig[i]);
                    // that._initDivData(pane,panesConfig[i]);              
                    // if (pane.nodeName.toLowerCase() != "script") {
                    // that._initPane(pane, panesConfig[i]);
                    // }
                });
            var _tempSize = that.element[that.sizingDomProperty]();
            this._resize();
            if(_tempSize != that.element[that.sizingDomProperty]()){
            	//如果初始化resize后，发现总大小在变动前后不一致（比如滚动条导致），则再次定位
            	this._resize();
            }
        },
        _initDiv:function(pane,config)
        {
        	var that =this;
            pane = $(pane);
            pane.data("pane", config ? config : {});
            pane.find('div.ui-pane-body').css("overflow", config.scrollable==true? "auto" : "visible");
             // pane.find('div').data("pane", config ? config : {})
             ...
             if(typeof(config)=="undefined")
            {
                ...
                if (config.scrollable) {
                    ...
                    {
                        config.header = "ui-pane-heading";
                    }
                    if(typeof(config.footer) == "undefined")
                    {
                        config.footer = "ui-pane-footer";
                    }
                }
             }
        },
        ...
        _resize: function() {
            var that = this,
                ...
                totalSize = element[that.sizingDomProperty]();

            if(!that.element.is(":visible")){
            	//不存在于界面上的不应该触发resize事件
            	return;
            }
            ...
            panes.css({
                position: "absolute",
                top: 0
            })[that.sizingDomProperty](function() {
            ...
            panes.each(function(i, pane) {
                var config = $(this).data("pane"),
                    headerHeight = 0,
                    footerHeight = 0,
                    totalHeight = $(pane).outerHeight(),
                    paneAttr = {};
                if (config.scrollable) {
                	if (config.header) {
                        headerHeight = $(pane).find("." + config.header.toString()).outerHeight();
                    }
                    if (config.footer) {
                        footerHeight = $(pane).find("." + config.footer.toString()).outerHeight();
                    }
                    $(pane).find(".ui-pane-body").css({
                        height: totalHeight - headerHeight - footerHeight
                    });
                    paneAttr = {
                        height: $(pane).find(".ui-pane-body").outerHeight(),
                        width: $(pane).find(".ui-pane-body").outerWidth()
                    };
                } else {
                    paneAttr = {
            ...
            that.options.afterResize.call(that, panesAttr);
        },
